...
function writeCurrentSeason() {
  ...
  const currentStyle = `<style>.row-toggle{align-items:center;background:transparent;border:0;color:var(--ink);cursor:pointer;display:inline-flex;font:inherit;font-weight:900;gap:8px;padding:0;text-align:left}.row-toggle span{color:var(--muted);display:inline-block;transition:transform 160ms ease}.row-toggle.open span{transform:rotate(180deg)}.manager-detail-row td{background:var(--bg);padding:16px}.live-roster-list{display:grid;gap:4px}.live-roster-player{align-items:center;border-bottom:1px solid var(--line);display:grid;font-size:12px;grid-template-columns:1fr auto;gap:10px;padding:5px 0}.live-roster-player:last-child{border-bottom:0}.live-roster-player strong{font-size:12px}.live-roster-player span{color:var(--muted);font-size:11px;font-weight:900}</style>`;
  const rows = standings.map((team) => {
    const roster = arr(season.finalRosters).filter((player) => player.teamId === team.teamId);
    const rosterHtml = roster.length
      ? `<div class="live-roster-list">${roster.map((player) => `<div class="live-roster-player"><strong>${esc(player.playerName)}</strong><span>${esc(currentRosterPosition(player))}</span></div>`).join("")}</div>`
