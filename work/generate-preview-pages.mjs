...
function writeHome() {
  ...
  const recapMatchupHtml = (label, row) => `<span><small>${label}</small><b>${row ? fmt(row.margin) : "-"}</b><strong>${esc(row ? matchupDetail(row) : "No matchup yet")}</strong><em>${esc(row ? `${managerName(row.home?.managerId)} vs ${managerName(row.away?.managerId)}` : "Scores appear when ESPN updates.")}</em></span>`;
