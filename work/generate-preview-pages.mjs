...
const playerbaseFile = JSON.parse(fs.readFileSync(path.join(root, "data", "playerbase", "espn-players.json"), "utf8"));
...
const lastUpdatedLabel = () => new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit", timeZoneName: "short" }).format(new Date());
...
function playerLookupForYear(year) {
  ...
  for (const [id, player] of espnPlayerbase) {
    if (!players.has(id) || String(players.get(id)).startsWith("Player ")) players.set(id, player.name);
  }
  return players;
}
...
function loadRosterMoves(year) {
  ...
  return arr(raw.topics).flatMap((topic, index) => {
    ...
    const playerName = (playerId, message) => {
      ...
      return players.get(playerId) || message.playerName || message.targetName || message.fullName || espnPlayerbase.get(playerId)?.name || (draftName && draftName !== "TBD" && !draftName.startsWith("Player ") ? draftName : undefined) || `Player ${message.targetId ?? "Unknown"}`;
    };
...
function designCss() {
  return `body{min-height:100vh;background:linear-gradient(90deg,rgba(31,111,82,.08) 1px,transparent 1px) 0 0/92px 92px,linear-gradient(0deg,rgba(31,111,82,.07) 1px,transparent 1px) 0 0/92px 92px,radial-gradient(circle at 18% 14%,rgba(185,133,36,.12),transparent 26%),radial-gradient(circle at 78% 8%,rgba(40,102,143,.09),transparent 24%),var(--bg)}.shell{display:block}aside{align-items:center;backdrop-filter:blur(18px);background:color-mix(in srgb,var(--panel) 82%,transparent);border-bottom:1px solid color-mix(in srgb,var(--line) 82%,transparent);border-right:0;box-shadow:0 12px 34px rgba(38,31,22,.07);display:flex;gap:22px;height:auto;padding:16px 28px;position:sticky;top:0;z-index:30}.brand{margin:0;min-width:226px}.brand b{font-size:26px;line-height:.96}.brand small{font-size:11px;font-weight:900;letter-spacing:.08em;margin-top:6px;text-transform:uppercase}.theme-toggle{margin:0}details.nav{background:transparent;border:0;flex:1;padding:0}details.nav summary{display:none}nav{align-items:center;display:flex;flex-wrap:wrap;gap:4px}.nav-group-label{color:var(--muted);font-size:11px;font-weight:900;letter-spacing:.08em;padding:0 6px;text-transform:uppercase;white-space:nowrap}.nav-divider{align-self:stretch;border-left:1px solid color-mix(in srgb,var(--line) 88%,transparent);border-top:0;display:block;margin:6px 8px;min-height:24px;width:1px}nav a{align-items:center;color:color-mix(in srgb,var(--ink) 82%,var(--muted));display:flex;font-size:14px;gap:8px;padding:9px 10px}nav a.active,nav a:hover{background:color-mix(in srgb,var(--green) 12%,transparent);color:var(--green)}main{margin:0 auto;max-width:1260px;padding:46px 42px 70px;width:100%}.champion-banners{margin:-18px 0 34px;padding:10px 4px 18px}.home-hero{grid-template-columns:minmax(0,.92fr) 340px;gap:18px;margin:18px 0 34px;max-width:980px}.grid{gap:14px}.grid.cols-2{max-width:900px}.grid.cols-3{grid-template-columns:minmax(250px,.92fr) minmax(220px,.72fr) minmax(270px,.92fr);max-width:1050px}.card{backdrop-filter:blur(12px);background:color-mix(in srgb,var(--panel) 90%,transparent);border-color:color-mix(in srgb,var(--line) 82%,transparent);overflow-x:auto;padding:15px}.section.card{max-width:1060px}.countdown-card{min-height:156px;padding:15px}.countdown-card strong{font-size:19px}.countdown-grid b,.mini-stats b{font-size:21px}.record b{font-size:27px}.member-grid{gap:8px}.member-tile{min-height:66px;padding:10px}.schedule-list{gap:8px}.schedule-matchup{padding:10px}.history-accent{background:linear-gradient(90deg,rgba(13,18,15,.78),rgba(13,18,15,.42)),radial-gradient(circle at 78% 26%,rgba(213,166,74,.4),transparent 22%),radial-gradient(circle at 92% 78%,rgba(185,133,36,.28),transparent 20%),linear-gradient(135deg,#14211c,#24352e 48%,#0c1210);border:1px solid rgba(244,234,212,.3);border-radius:8px;box-shadow:0 18px 50px rgba(8,18,14,.14);color:#fff;margin-bottom:22px;overflow:hidden;padding:24px;position:relative}.history-accent:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,transparent 0 72%,rgba(213,166,74,.18) 72% 72.4%,transparent 72.4%),repeating-linear-gradient(0deg,transparent 0 18px,rgba(255,255,255,.04) 18px 19px);pointer-events:none}.history-accent>*{position:relative;z-index:1}.history-accent h1{color:#fff;margin-bottom:0}.history-accent .eyebrow,.history-accent p{color:#f4ead4}@media(max-width:900px){aside{align-items:stretch;display:grid;padding:18px}.brand{min-width:0}details.nav{background:var(--panel);border:1px solid var(--line);padding:8px}details.nav summary{display:flex}.theme-toggle{margin-top:12px}nav{align-items:stretch;display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}nav a{font-size:13px;gap:7px;padding:10px 8px}.nav-group-label{grid-column:1/-1;padding:10px 4px 2px}.nav-divider{border-left:0;border-top:1px solid var(--line);grid-column:1/-1;margin:6px 4px;min-height:0;width:auto}main{padding:24px 18px 58px}.history-accent{margin-bottom:18px;padding:18px}.history-accent h1{font-size:clamp(30px,10vw,44px)}.champion-banners{gap:12px;margin:-6px 0 22px;padding:8px 0 14px}.champion-banner{border-width:3px;height:146px;min-width:104px;padding:18px 8px 34px;width:104px}.champion-banner.latest{height:164px;min-width:118px;width:118px}.champion-banner span,.champion-banner small{font-size:10px}.champion-banner strong{font-size:13px}.champion-banner.latest strong{font-size:14px}.champion-banner .banner-year{font-size:25px}.champion-banner.latest .banner-year{font-size:29px}.countdown-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.table,table{min-width:680px}.home-hero,.grid.cols-2,.grid.cols-3{grid-template-columns:1fr;max-width:none}}@media(max-width:560px){nav{grid-template-columns:1fr}.brand b{font-size:23px}.brand small{font-size:10px}.home-hero{margin-top:8px}.site-version{left:10px;right:auto;text-align:left}}`;
}
...
function writeHome() {
  ...
  const recapHtml = (label, row) => `<span><small>${label}</small><b>${row ? fmt(row.total) : "-"}</b><strong>${esc(row ? managerName(row.team.managerId) : "No scores yet")}</strong><em>${esc(row ? "Manager total" : "Scores appear when ESPN updates.")}</em></span>`;
  ...
  const matchupDetail = (row) => row ? `${managerName(row.home?.managerId)} ${fmt(row.homeScore)} - ${fmt(row.awayScore)} ${managerName(row.away?.managerId)}` : "Scores appear when ESPN updates.";
  const recapMatchupHtml = (label, row) => `<span><small>${label}</small><b>${row ? fmt(row.margin) : "-"}</b><strong>${esc(row ? matchupDetail(row) : "No matchup yet")}</strong><em>${esc(row ? "Manager matchup" : "Scores appear when ESPN updates.")}</em></span>`;
  const extraRecapHtml = `${recapMatchupHtml(recapProjected ? "Biggest projected edge" : "Biggest blowout", biggestEdge)}${recapMatchupHtml("Closest matchup", closestMatchup)}<span><small>Projection miss</small><b>${projectionMiss ? fmt(projectionMiss.miss) : "-"}</b><strong>${esc(projectionMiss ? managerName(projectionMiss.team.managerId) : "Pending finals")}</strong><em>${esc(projectionMiss ? "Manager projection" : "Available after final scores.")}</em></span>`;
...
function writeSchedule() {
  const season = seasons.find((item) => item.year === currentYear);
  if (!season) {
    write("schedule.html", shell("Chudgate Schedule", "schedule", `<header class="history-accent"><h1>Schedule</h1></header><section class="card"><p class="muted">No 2026 schedule export found.</p></section>`));
    return;
  }
  ...
  write("schedule.html", shell("Chudgate Schedule", "schedule", `${scheduleStyle}<header class="history-accent"><h1>Schedule</h1></header><section class="card"><label class="select-field"><span>Week</span><select data-schedule-week-select>${weekOptions}</select></label></section>${panels}${lookup}${script}`));
}
...
function writeHistoryWithRosters() {
  ...
  write("history.html", shell("Chudgate League History", "history", `${historyStyle}<header class="history-accent"><h1>League History</h1></header><section><div class="picker">${buttons}</div>${panels}</section>${scriptPicker(rosterScript)}`));
}
...
function writeRivalries() {
  ...
  write("rivalries.html", shell("Chudgate Rivalries", "rivalries", `<header class="history-accent"><h1>Rivalries</h1></header><section><div class="picker">${buttons}</div>${panels}</section>${scriptPicker(sortScript)}`));
}
...
function writeRecords() {
  ...
  write("records.html", shell("Chudgate Awards", "records", `<header class="history-accent"><h1>Awards</h1></header><section class="grid">${records.map(([label,value,detail])=>`<article class="card record"><span class="muted">${esc(label)}</span><b>${esc(value)}</b><small>${esc(detail)}</small></article>`).join("")}</section>`));
}
...
function writeDrafts() {
  ...
  write("drafts.html", shell("Chudgate Drafts", "drafts", `<header class="history-accent"><h1>Drafts</h1></header><section><div class="picker">${buttons}</div>${panels}</section>${scriptPicker(draftScript)}`));
}
...
function writePlayerLookup() {
  ...
  write("player-lookup.html", shell("Chudgate Player Lookup", "player-lookup", `${style}<header class="history-accent"><h1>Player Lookup</h1></header><article class="card"><div class="row-between"><h2>Find Player</h2><span class="tag">Draft profile</span></div><label class="select-field lookup-field"><span>Search Player</span><input data-player-profile-search placeholder="Type a player name"></label><label class="select-field lookup-field"><span>Select Player</span><select data-player-profile-select>${options}</select></label><p class="muted" data-player-profile-empty style="display:none">No matching player found.</p></article>${profileArticles}${script}`));
}
...
function writeTrades() {
  ...
  const renderAddDrops = (activities) => groupedRows(activities).map((group, groupIndex) => `<details class="card move-date-group" data-date-group ${groupIndex === 0 ? "open" : ""}><summary class="move-date-summary"><div><h2>${esc(group.label)}</h2><span class="tag green">${currentYear}</span></div><span class="tag" data-group-count>${group.rows.length} player moves</span></summary><table><thead><tr><th>Player</th><th>Move</th><th>Manager</th><th>FAAB</th><th>Weeks Since</th><th>Points Since</th></tr></thead><tbody>${group.rows.map(({ activity, move, index }) => { const impact = impactFor(activity.id, index); const managers = managerIdsForMove(move).join("|"); const actionClass = move.action === "Added" ? "added" : "dropped"; return `<tr data-move-row data-row-order="${index}" data-managers="${esc(managers)}" data-team-sort="${esc(teamSortName(move))}"><td><strong>${esc(move.player)}</strong>${projectionNote(impact)}</td><td><span class="move-action ${actionClass}">${esc(move.action)}</span></td><td>${esc(move.manager)}</td><td>${move.bidAmount !== undefined ? `$${esc(move.bidAmount)}` : "-"}</td><td>${impact?.weeksTracked || "-"}</td><td>${move.action === "Added" ? impactCell(impact) : "-"}</td></tr>`; }).join("")}</tbody></table></details>`).join("");
  ...
  write("trades.html", shell("Chudgate Roster Moves", "trades", `${style}<header class="history-accent"><h1>Roster Moves</h1></header>${impactSummary}<div class="seg"><button class="active" data-roster-tab="trades" type="button">Trades</button><button data-roster-tab="add-drop" type="button">Add/Drop</button></div><div data-roster-panel="trades">${tradeContent}</div><div data-roster-panel="add-drop" style="display:none">${addDropContent}</div>${script}`));
}

function writeManagers() {
  const managerStyle = `<style>.row-toggle{align-items:center;background:transparent;border:0;color:var(--ink);cursor:pointer;display:inline-flex;font:inherit;font-weight:900;gap:8px;padding:0;text-align:left}.row-toggle span{color:var(--muted);display:inline-block;transition:transform 160ms ease}.row-toggle.open span{transform:rotate(180deg)}.manager-detail-row td{background:var(--bg);padding:16px}.placement-line-chart{max-width:680px;overflow-x:auto;padding:4px 0 0}.placement-line-chart svg{display:block;min-width:360px;width:100%}.chart-grid-line{stroke:var(--line);stroke-dasharray:4 5}.placement-line{fill:none;stroke:var(--green);stroke-linecap:round;stroke-linejoin:round;stroke-width:1.5}.placement-dot{fill:var(--panel);stroke:var(--green);stroke-width:2}.placement-dot.champion{fill:var(--gold);stroke:var(--gold)}.placement-value{fill:var(--ink);font-size:10px;font-weight:900}.placement-year{fill:var(--ink);font-size:10px;font-weight:900}</style>`;
  const rows = careers.map((row) => `<tr data-manager-row="${esc(row.manager.id)}"><td><button class="row-toggle" type="button" data-manager-toggle="${esc(row.manager.id)}" aria-expanded="false"><span>▾</span>${esc(row.manager.displayName)}</button><a class="cell-note text-button" href="manager-${esc(managerSlug(row.manager.id))}.html">Open profile</a></td><td><span class="tag ${active2026.has(row.manager.id) ? "green" : "red"}">${esc(yearLabels.get(row.manager.id))}</span></td><td>${row.seasons}</td><td>${row.wins}-${row.losses}${row.ties?`-${row.ties}`:""}</td><td>${pct(row.winPct)}</td><td>${row.championships}</td><td>${row.topThreeFinishes}</td><td>${row.playoffAppearances}</td><td>${row.playoffWins}-${row.playoffLosses}${row.playoffTies?`-${row.playoffTies}`:""}</td><td>${row.averageFinish?.toFixed(1) ?? "-"}</td><td>${fmt(row.pointsFor)}</td></tr><tr class="manager-detail-row" data-detail-for="${esc(row.manager.id)}" style="display:none"><td colspan="11">${placementChart(row.manager.id)}</td></tr>`).join("");
  ...
  write("managers.html", shell("Chudgate Manager Stats", "managers", `${managerStyle}<header class="history-accent"><h1>Manager Stats</h1></header><section class="card"><table><thead><tr><th>Manager</th><th>Years Active</th><th><button data-sort="seasons">Seasons</button></th><th><button data-sort="wins">Regular Season Record</button></th><th><button class="active" data-sort="winPct">Win %</button></th><th><button data-sort="titles">Titles</button></th><th><button data-sort="top3">Top 3 Finishes</button></th><th><button data-sort="playoffs">Playoffs</button></th><th><button data-sort="playoffRecord">Winners Bracket Record</button></th><th><button data-sort="avg">Avg Finish</button></th><th><button data-sort="pf">PF</button></th></tr></thead><tbody>${rows}</tbody></table></section>${script}`));
}

function writeManagerProfiles() {
  const lowRows = weeklyLowTeamRows(2026);
  const profileStyle = `<style>.profile-back{display:inline-flex;margin-bottom:12px}.profile-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.profile-grid .record{margin:0}.profile-seasons{display:grid;gap:12px}.profile-rivals{display:grid;gap:10px}.profile-rival{align-items:center;border:1px solid var(--line);border-radius:8px;display:flex;justify-content:space-between;padding:12px}.placement-line-chart{max-width:680px;overflow-x:auto;padding:4px 0 0}.placement-line-chart svg{display:block;min-width:360px;width:100%}.chart-grid-line{stroke:var(--line);stroke-dasharray:4 5}.placement-line{fill:none;stroke:var(--green);stroke-linecap:round;stroke-linejoin:round;stroke-width:1.5}.placement-dot{fill:var(--panel);stroke:var(--green);stroke-width:2}.placement-dot.champion{fill:var(--gold);stroke:var(--gold)}.placement-value{fill:var(--ink);font-size:10px;font-weight:900}.placement-year{fill:var(--ink);font-size:10px;font-weight:900}@media(max-width:900px){.profile-grid{grid-template-columns:1fr}.profile-rival{display:block}}</style>`;
...
function placementChart(managerId) {
  ...
  const maxPlacement = Math.max(...seasons.map((season) => season.teams.length), ...valid.map((point) => point.placement ?? 0), 1);
  const width = Math.max(360, points.length * 58);
  const height = 122;
  const paddingX = 28;
  const top = 22;
  const bottom = 86;
  ...
  return `<div class="placement-line-chart"><svg viewBox="0 0 ${width} ${height}" role="img"><line class="chart-grid-line" x1="${paddingX}" x2="${width - paddingX}" y1="${top}" y2="${top}"></line><line class="chart-grid-line" x1="${paddingX}" x2="${width - paddingX}" y1="${bottom}" y2="${bottom}"></line>${linePath ? `<path class="placement-line" d="${linePath}"></path>` : ""}${plotted.map((point) => `<g>${point.y !== undefined ? `<circle class="${point.placement === 1 ? "placement-dot champion" : "placement-dot"}" cx="${point.x}" cy="${point.y}" r="4"></circle><text class="placement-value" x="${point.x}" y="${point.y - 8}" text-anchor="middle">${esc(point.placement)}</text>` : ""}<text class="placement-year" x="${point.x}" y="108" text-anchor="middle">${point.year}</text></g>`).join("")}</svg></div>`;
}
